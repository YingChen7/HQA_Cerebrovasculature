...
for isubj=1:length(subj_folders)
    ...
    for ibrod=1:length(brd_labels)
        brodmann_feature(1, ibrod, isubj)=sum(cast(brodmann_img == brd_labels(ibrod), 'uint8')...
            .* cortical_mask .* warped_brain, 'all') * voxelsize;

        brodmann_feature(2, ibrod, isubj)=sum(cast(brodmann_img == brd_labels(ibrod), 'uint8')...
