...
class DataLoader3D(SlimDataLoaderBase):
    def __init__(self, data, patch_size, final_patch_size, batch_size, has_prev_stage=False,
        ...
        self.data_shape, self.seg_shape, self.subsource_shape = self.determine_shapes()
    ...
    def determine_shapes(self):
        ...
        subsource_shape = (self.batch_size, 4)
        return data_shape, seg_shape, subsource_shape

    def generate_train_batch(self):
        selected_keys = np.random.choice(self.list_of_keys, self.batch_size, True, None)
        data = np.zeros(self.data_shape, dtype=np.float32)
        seg = np.zeros(self.seg_shape, dtype=np.float32)
        subsource = np.zeros(self.subsource_shape)
        case_properties = []
        for j, i in enumerate(selected_keys):
            ...
            if 'AB' in i:
                subsource[j] = np.array([1, 0, 0, 0])
            elif 'IOP' in i:
                subsource[j] = np.array([0, 1, 0, 0])
            elif 'HH' in i: 
                subsource[j] = np.array([0, 0, 1, 0])
            elif 'Guys' in i:
                subsource[j] = np.array([0, 0, 0, 1])
            else:
                print('Can not get the modality')
        ...
        return {'data': data, 'seg': seg, 'properties': case_properties, 'keys': selected_keys, 'subsource': subsource}
