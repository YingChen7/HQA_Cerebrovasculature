# -*- coding: utf-8 -*-
"""
Created on Fri Oct 29 16:59:54 2021
...
class SSLDataLoader(SlimDataLoaderBase):
    def __init__(self, data, patch_size, batch_size, has_prev_stage=False,
                 oversample_foreground_percent=0.0, memmap_mode="r"):
        """
        ...
        """
        super(SSLDataLoader, self).__init__(data, batch_size, None)
        ...
        self.data_shape, self.seg_shape, self.coord_shape = self.determine_shapes()
    ...
    def determine_shapes(self):
        ...
        coord_shape = (self.batch_size, 6)
        return data_shape, seg_shape, coord_shape

    def generate_train_batch(self):
        self.selected_keys = np.random.choice(self.list_of_keys, self.batch_size, True, None)
        img = self.generate_train_batch_pair()
        img2 = self.generate_train_batch_pair()
        
        return img, img2
        
        
    def generate_train_batch_pair(self):
        data = np.zeros(self.data_shape, dtype=np.float32)
        seg = np.zeros(self.seg_shape, dtype=np.float32)
        coord = np.zeros(self.coord_shape, dtype = np.float32)
        case_properties = []
        for j, i in enumerate(self.selected_keys):
            ...
            coord[j] = np.array([float(valid_bbox_x_lb)/(shape[0]-1), float(valid_bbox_y_lb)/(shape[1]-1), float(valid_bbox_z_lb)/(shape[2]-1),
                                 float(valid_bbox_x_ub)/(shape[0]-1), float(valid_bbox_y_ub)/(shape[1]-1), float(valid_bbox_z_ub)/(shape[2]-1)]) 

            seg[j, 0] = case_all_data[-1:]
            if seg_from_previous_stage is not None:
                seg[j, 1] = seg_from_previous_stage
                
         # data augmentation:
        _, _, data, coord = get_batch_augmentation(data, self.patch_size, coord = coord)
        return {'data': data, 'seg': seg, 'properties': case_properties, 'keys': self.selected_keys, 'coord': coord}
