...
def regression_loss(q, k, coord_q, coord_k, pos_ratio=0.5, anatomy=True):
    ...
    #print(len((pos_mask>0.1).nonzero()))
...
class VessPro(BaseModel):
    def __init__(self, base_encoder, params, slc_layers=1):
        ...
        self.projector = []
        self.projector_k = []
        for i in range(self.slc_layers):
            self.projector.append(Proj_Head(in_dim=params['encoder_channels'][i], inner_dim=2*params['encoder_channels'][i], out_dim = params['encoder_channels'][i]))
            self.projector_k.append(Proj_Head(in_dim=params['encoder_channels'][i], inner_dim=2*params['encoder_channels'][i], out_dim = params['encoder_channels'][i]))
        
        for param_q, param_k in zip(self.encoder.parameters(), self.encoder_k.parameters()):
            param_k.data.copy_(param_q.data)  # initialize
            param_k.requires_grad = False  # not update by gradient
        
        self.projector = nn.ModuleList(self.projector)
        self.projector_k = nn.ModuleList(self.projector_k)    
        ...
        for i in range(self.slc_layers):
             for param_q, param_k in zip(self.projector[i].parameters(), self.projector_k[i].parameters()):
                 param_k.data.copy_(param_q.data)
                 param_k.requires_grad = False
        ...
        for i in range(self.slc_layers):
            if self.VessPro_transform_layer == 0:
                self.value_transform.append(Identity())
            elif self.VessPro_transform_layer == 1:
                self.value_transform.append(nn.Conv3d(in_channels=params['encoder_channels'][i], out_channels=params['encoder_channels'][i], kernel_size=1, stride=1, padding=0, bias=True))
            elif self.VessPro_transform_layer == 2:
                self.value_transform.append(MLP3d(in_dim=params['encoder_channels'][i], inner_dim=params['encoder_channels'][i], out_dim=params['encoder_channels'][i]))
            else:
                raise NotImplementedError
        
        self.value_transform = nn.ModuleList(self.value_transform)        
        ...
        if self.VessPro_ins_loss_weight > 0.:
            ...
            '''
            self.avgpool = nn.AvgPool3d((7, 7, 6), stride=1)
    ...
    def featprop(self, feat, feat_value):
        N, C, H, W, D = feat.shape
        
        # Value transformation (change the place of value transform)
    ...
    def forward(self, im_1, im_2, coord1, coord2):
        ...
        skips1 = self.encoder(im_1)  # queries: 
        pred_1 = []

        for i in range(self.slc_layers):
                
            proj_1 = self.projector[i](skips1[self.slc_layers-(i + 1)])
            #pred_1 = self.featprop(feat_1)
            feat_value_1 = F.normalize(self.value_transform[i](proj_1), dim=1)
            pred_1.append(F.normalize(self.featprop(proj_1, feat_value_1), dim=1))
        
        skips2 = self.encoder(im_2)
        pred_2 = []
        for i in range(self.slc_layers):
            
            proj_2 = self.projector[i](skips2[self.slc_layers-(i + 1)])
            #pred_2 = self.featprop(feat_2)         
            feat_value_2 = F.normalize(self.value_transform[i](proj_2), dim=1)
            pred_2.append(F.normalize(self.featprop(proj_2, feat_value_2), dim=1))
                
        if self.VessPro_ins_loss_weight > 0.:
            proj_instance_1 = self.projector_instance(skips1[0])
            ...
            proj_instance_2 = self.projector_instance(skips2[0])
        ...
        with torch.no_grad():  # no gradient to keys
            ...
            '''
            skips1_ng = self.encoder_k(im_1)  # keys: NxC
            proj_1_ng = []
            for i in range(self.slc_layers):               
                proj_1_ng.append(F.normalize(self.projector_k[i](skips1_ng[self.slc_layers-(i + 1)]), dim=1))
            
            skips2_ng = self.encoder_k(im_2)  # keys: NxC
            proj_2_ng = []
            for i in range(self.slc_layers):
                proj_2_ng.append(F.normalize(self.projector_k[i](skips2_ng[self.slc_layers-(i + 1)]), dim=1))
            
            if self.VessPro_ins_loss_weight > 0.:
                proj_instance_1_ng = self.projector_instance_k(skips1_ng[0])
                proj_instance_1_ng = F.normalize(self.avgpool(proj_instance_1_ng).view(proj_instance_1_ng.size(0), -1),
                                                 dim=1)

                proj_instance_2_ng = self.projector_instance_k(skips2_ng[0])
        ...
        loss = 0
        for i in range(self.slc_layers):
            loss = loss + regression_loss(pred_1[i], proj_2_ng[i], coord1, coord2, self.VessPro_pos_ratio) \
            + regression_loss(pred_2[i], proj_1_ng[i], coord2, coord1, self.VessPro_pos_ratio)
