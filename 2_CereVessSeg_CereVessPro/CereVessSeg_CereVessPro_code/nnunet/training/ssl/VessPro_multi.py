...
class VessPro(BaseModel):
    def __init__(self, base_encoder, params, slc_layers=1):
        ...
        for i in range(self.slc_layers):
            self.projector.append(Proj_Head(in_dim=params['encoder_channels'][-(i + 1)], inner_dim=2*params['encoder_channels'][-(i + 1)], out_dim = params['encoder_channels'][-(i + 1)]))
            self.projector_k.append(Proj_Head(in_dim=params['encoder_channels'][-(i + 1)], inner_dim=2*params['encoder_channels'][-(i + 1)], out_dim = params['encoder_channels'][-(i + 1)]))
        ...
        for i in range(self.slc_layers):
            if self.VessPro_transform_layer == 0:
                ...
                self.value_transform.append(nn.Conv3d(in_channels=params['encoder_channels'][-(i + 1)], out_channels=params['encoder_channels'][-(i + 1)], kernel_size=1, stride=1, padding=0, bias=True))
            elif self.VessPro_transform_layer == 2:
                self.value_transform.append(MLP3d(in_dim=params['encoder_channels'][-(i + 1)], inner_dim=params['encoder_channels'][-(i + 1)], out_dim=params['encoder_channels'][i]))
            else:
                raise NotImplementedError
    ...
    def forward(self, im_1, im_2, coord1, coord2):
        ...
        skips1, feat_1 = self.encoder(im_1)  # queries: 
        pred_1 = []

        for i in range(self.slc_layers):
                
            proj_1 = self.projector[i](skips1[-(i + 1)])
            #pred_1 = self.featprop(feat_1)
        ...
        skips2, feat_2 = self.encoder(im_2)
        pred_2 = []
        for i in range(self.slc_layers):
            
            proj_2 = self.projector[i](skips2[-(i + 1)])
            #pred_2 = self.featprop(feat_2)         
        ...
        with torch.no_grad():  # no gradient to keys
            ...
            '''
            skips1_ng, feat_1_ng = self.encoder_k(im_1)  # keys: NxC
            proj_1_ng = []
            for i in range(self.slc_layers):               
                proj_1_ng.append(F.normalize(self.projector_k[i](skips1_ng[-(i + 1)]), dim=1))
            
            skips2_ng, feat_2_ng = self.encoder_k(im_2)  # keys: NxC
            proj_2_ng = []
            for i in range(self.slc_layers):
                proj_2_ng.append(F.normalize(self.projector_k[i](skips2_ng[-(i + 1)]), dim=1))
