...
'''   
class HAE_loss(nn.Module):
    def __init__(self,  soft_dice_kwargs, ce_kwargs, aggregate="sum", square_dice=False, weight_ce=1, weight_dice=1,
                 log_dice=False, ignore_label=None):
        super(HAE_loss, self).__init__()
        if ignore_label is not None:
            assert not square_dice, 'not implemented'
            ce_kwargs['reduction'] = 'none'
        self.log_dice = log_dice
    ...
    def forward(self, net_output, target):
        ...
        # print(target.shape)
        # foreground_ratio = 1.0 / (torch.sum(target) + 1e-5)
        # background_ratio = 1.0 / (torch.sum(1-target) + 1e-5) weight = torch.Tensor([background_ratio, foreground_ratio]).cuda()
        ce_loss = nn.functional.cross_entropy(net_output, target, reduction='none')
        
        bin_seg = torch.argmax(softmax_helper(net_output), dim = 1)
        
        mask_fp = (bin_seg * (1-target)+ 1e-5)
        mask_fn = ((1-bin_seg)*target + 1e-5)
        
        term1 = ce_loss.mean()
        # term2 = (1.0 / (torch.sum(mask_fp) + 1e-5))* ((ce_loss * mask_fp).sum())
        term2 = (ce_loss * mask_fp).mean()
        # term3 = (1.0 / (torch.sum(mask_fn) + 1e-5))* ((ce_loss * mask_fn).sum())
        term3 = (ce_loss * mask_fn).mean()
        
        a_ce_loss = term1+term2+term3
        
        # distance = torch.abs((bin_seg - target))
        
        # d_ce_loss = (distance * ce_loss).mean() 
        
        # print(d_ce_loss)
        result = self.weight_ce * a_ce_loss + self.weight_dice * dc_loss
...
class HAE_loss(nn.Module):
    ...
    def forward(self, net_output, target):
        ...
        distance = torch.abs(( net_output[:, 0].long()- target))
        
        dis_ce_loss = distance * ce_loss
        
        term1 = dis_ce_loss.mean()
        # term2 = (1.0 / (torch.sum(mask_fp) + 1e-5))* ((ce_loss * mask_fp).sum())
        term2 = (ce_loss * mask_fp).mean()
        # term3 = (1.0 / (torch.sum(mask_fn) + 1e-5))* ((ce_loss * mask_fn).sum())
        term3 = (ce_loss * mask_fn).mean()
