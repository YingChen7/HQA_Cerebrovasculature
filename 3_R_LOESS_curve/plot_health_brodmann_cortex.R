...
health_brodmann_file <- "./data/subj_cortical_brodmann_params_IXIOAS07.csv" # brodmann atlas parameters
...
# Compute P values
areas_names <- names(health_brodmann_all)[1:41] # names of all sub-region in the brodmann atlas
...
for (idx in 1:length(areas_number)) {
  ...
  plist[[idx]]<-ggplot(data=health_brodmann_all, mapping=aes_string(x="age", y=name, color="sex", shape="sex"))+
    ...
    xlab("Age")+
    ylab(str_c(areas_names[areas_number[idx]], " cortical\n volume,", " mm³"))+
    theme_minimal()+
  ...
  pdf(sprintf("cortical_volume_%s.pdf", areas_names[areas_number[idx]]),
